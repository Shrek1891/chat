...
const useMembership = (): IuseServer => {
    ...
    const joinServer = async (serverId: number): Promise<void> => {
        setIsLoading(true);
        try {
            await jwtAxios.post(`${API_URL}/membership/${serverId}/membership/`, {}, {withCredentials: true})
            setIsLoading(false)
    ...
    const leaveServer = async (serverId: number): Promise<void> => {
        setIsLoading(true);
        try {
            await jwtAxios.delete(`${API_URL}/membership/${serverId}/membership/remove_member/`, {withCredentials: true});
            setIsLoading(false);
    ...
    const isMember = async (serverId: number): Promise<any> => {
        setIsLoading(true);
        try {
            const response = await jwtAxios.get(`${API_URL}/membership/${serverId}/membership/is_member/`, {withCredentials: true});
            setIsLoading(false);
